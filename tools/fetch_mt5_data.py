...
def fetch(date_from_override: str | None = None, date_to_override: str | None = None,
    ...
    try:
        ...
        date_from = date_from_override or m.get("date_from")
        date_to = date_to_override or m.get("date_to")
        ...
        out_rel = out_override or m.get("out_file") or cfg.BACKTEST["data_file"]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Export MT5 OHLC candles to CSV")
    parser.add_argument("--from", dest="date_from", help="UTC start date, YYYY-MM-DD")
    parser.add_argument("--to", dest="date_to", help="UTC end date, YYYY-MM-DD")
    parser.add_argument("--out", help="output path relative to project root")
    args = parser.parse_args()
    if bool(args.date_from) != bool(args.date_to):
        parser.error("--from and --to must be supplied together")
    sys.exit(fetch(args.date_from, args.date_to, args.out))
